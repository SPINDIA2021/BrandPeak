...
import com.spindiabrand.api.ApiService;
...
import com.spindiabrand.database.BusinessDao;
import com.spindiabrand.database.UserDao;
import com.spindiabrand.database.UserLoginDao;
...
public class UserRepository {
    ...
    public LiveData<Resource<ReferWithdrawalDetail>> getReferWithdrawalDetail(String apiKey, String userId){
        return new NetworkBoundResource<ReferWithdrawalDetail, ReferWithdrawalDetail>() {
            @Override
            protected void saveCallResult(@NonNull ReferWithdrawalDetail item) {

                try {
                    db.runInTransaction(() -> {
                        userDao.deleteReferWithdrawalDetail();
                        userDao.insertWithdrawalRefer(item);
                    });
                }catch (Exception e){
            ...
            @Override
            protected boolean shouldFetch(@Nullable ReferWithdrawalDetail data) {
                return Config.IS_CONNECTED;
            }

            @NonNull
            @Override
            protected LiveData<ReferWithdrawalDetail> loadFromDb() {
                return userDao.getReferWithdrawlDetail();
            }

            @NonNull
            @Override
            protected LiveData<ApiResponse<ReferWithdrawalDetail>> createCall() {
                return ApiClient.getApiService().getReferWithdrawalDetails(apiKey, userId);
            }
        }.asLiveData();
    }
